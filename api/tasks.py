...
def process_settlements():
    ...
    try:
        ...
        for order in orders_to_settle:
            logger.info(f"Settling Order #{order.id} for User {order.user.username}, Amount: {order.total_price}")
        ...
        for seller, total_earnings in seller_earnings.items():
            logger.info(f"Mock sending message to Seller {seller.username}: 'Your total earnings are ${total_earnings:.2f}'")
