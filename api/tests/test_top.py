...
def test_top_selling_products_all_sellers(api_client_auth, admin, user, create_product, create_order_item, order):
    product1 = create_product("Product 1", 10.00, admin)
    product2 = create_product("Product 2", 20.00, admin)

    OrderItem.objects.create(order=order, product=product1, quantity=5, unit_price=product1.price)
    OrderItem.objects.create(order=order, product=product2, quantity=3, unit_price=product2.price)
...
@pytest.mark.django_db
def test_top_selling_products_single_seller(api_client_auth, admin, create_product, create_order_item, order):
    products = [create_product(f"Product {i}", 10.00 * i, admin) for i in range(1, 4)]
    for i, product in enumerate(products, start=1):
        OrderItem.objects.create(order=order, product=product, quantity=i, unit_price=product.price)
...
@pytest.mark.django_db
def test_top_rated_products_all_sellers(api_client_auth, admin, user, create_product, create_review):
    product1 = create_product("Product 1", 10.00, admin)
    product2 = create_product("Product 2", 20.00, admin)
...
@pytest.mark.django_db
def test_top_rated_products_single_seller(api_client_auth, admin, create_product, create_review,user):
    products = [create_product(f"Product {i}", 10.00 * i, admin) for i in range(1, 4)]
    ratings = [5, 3, 4]
...
@pytest.mark.django_db
def test_top_selling_products_with_limit(api_client_auth, admin, create_product, create_order_item, order):
...
@pytest.mark.django_db
def test_top_rated_products_with_limit(api_client_auth, admin, create_product, create_review,user):
    products = [create_product(f"Product {i}", 10.00 * i, admin) for i in range(1, 6)]
    for i, product in enumerate(products, start=1):
