...
def test_get_user_reports(user,product):
    report = Report.objects.create(user=user, report_type='product', rid=product.id, message='Test message')
...
def test_create_report(user,order):
    report = create_report(user=user, report_type='order', rid=order.id, message='New message')
...
def test_list_reports(api_client_auth, user,product):
    Report.objects.create(user=user, report_type='product', rid=product.id, message='Test message')
...
def test_retrieve_report(api_client_auth, user):
    report = Report.objects.create(user=user, report_type='product', rid=1, message='Test message')
